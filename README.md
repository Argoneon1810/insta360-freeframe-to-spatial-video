...
- FFMPEG가 PATH에 등록되어 있어야 합니다.
  - ffmpeg을 호출할 수 있는지 확인한 후, 필요시 커스텀 경로를 지정할 수 있도록 확장할 계획입니다.
...
- FFMPEG must be registered in the system PATH.
  - I plan to expand this to verify if ffmpeg can be called, and allow users to specify a custom path if necessary.

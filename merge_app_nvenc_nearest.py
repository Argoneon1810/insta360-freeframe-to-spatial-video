...
class MergeApp:
    def __init__(self, root, debug=False, test_len=5):
        ...
        self.root.title("Insta360 Merge Converter")
        
        self.files = []
        
        # 파일 선택 UI
        file_frame = tk.LabelFrame(root, text="입력 파일 (목록 순서대로 병합됨)")
        ...
        # 실행 버튼
        self.btn_run = tk.Button(root, text="Merge 실행", command=self.run, height=2, bg="lightblue")
    ...
    def run(self):
        if len(self.files) < 2:
            messagebox.showwarning("경고", "Merge 모드에서는 최소 2개 이상의 파일이 필요합니다.")
            return
    ...
    def process(self):
        try:
            target_dir = os.path.dirname(self.files[0]) if self.same_path_var.get() else self.out_path_var.get()
            if not target_dir: target_dir = os.path.dirname(self.files[0])
            
            base_name = os.path.splitext(os.path.basename(self.files[0]))[0] + "_merged"
            out_file = get_unique_filepath(target_dir, base_name, ".mkv")
            
            cmd = ["ffmpeg", "-threads", "16", "-filter_threads", "8"]
            for f in self.files:
                cmd.extend(["-hwaccel", "cuda", "-i", f])
            
            concat_str = "".join([f"[{i}:v][{i}:a]" for i in range(len(self.files))])
            filter_complex = f'{concat_str}concat=n={len(self.files)}:v=1:a=1[v_out][a_out];[v_out]v360=input=fisheye:output=equirect:ih_fov=180:iv_fov=180:h_fov=180:v_fov=180:w=4096:h=4096:interp=nearest,pad=8192:4096:2048:0:black,format=yuv420p[v_final]'
            ...
            messagebox.showinfo("완료", "Merge 작업이 완료되었습니다.")
            
        except Exception as e:
            messagebox.showerror("오류", f"작업 중 오류 발생:\n{e}")
        finally:
            self.btn_run.config(state="normal", text="Merge 실행")
